...
func (stats *BasicStats) LoadCharacter(sp *SpawnPoint, cons *Consumables) *Character {
	...
	if (*sp).Behavior == "Player" {
		...
		base.Body = common.BRandNewStream(phys, 4) // Entro
		base.Body.ScaleTo(3) // Len
		base.SproutAStream(true)
	...
	buffer.XYZ = [3]int{ base[0]+common.ChancedRound(x*mod), base[1]+common.ChancedRound(y*mod), base[2]+common.ChancedRound(z*mod) }
